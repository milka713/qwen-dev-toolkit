...
- **ON** (note the target % in the result): confirm test-first mode is on at that coverage target and pinned in `QWEN.md`. From now on, build **test-first** (write the failing test, watch it fail, implement, watch it pass, refactor). Every feature you (or a delegated `implementer`) build must ship tests covering its new/changed code — edge cases and error paths, not just the happy path — and you measure coverage with the project's real tool (if none is configured, set up the ecosystem's standard one — `pytest-cov`, `jest --coverage`, `go test -cover`), must hit the target, and report the actual measured number. Nothing is "done" until its tests exist and pass. If there is current work in `.qwen/PROGRESS.md`, offer to back-fill tests for what's already built.
