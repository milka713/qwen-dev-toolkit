...
- **"reset done"**: confirm exactly what was reset and in which scope. If settings changed (global scope), remind them to restart qwen-code / start a new session for those to take effect. Mention it is reversible with `/toolkit-reset undo` (one level back), and that they can re-apply any toggle per-project again with `/dev`, `/cover`, `/bro`, `/maxagents`, `/versioning`, `/reality`.
- **"undo done"**: confirm what was restored and the scope. For an undo of a global reset, remind them to restart qwen-code / start a new session for the settings to take effect. (`undo` reverts only the most recent `confirm`, one level — run it from the same project you reset, so it can rewrite that project's `QWEN.md`.)
- **"nothing to reset"**: tell them that scope already matches the current version's defaults — nothing to clean up.
- **"nothing to undo"**: explain that `undo` only works right after a `/toolkit-reset confirm` that actually changed something (it consumes the backup, so it is one level deep).
