#!/usr/bin/env bash
# /main-push backend — deterministic, user-only authorization for ONE main/master operation.
...
# merge+push release, not just one command); revoke early with /main-push off.
...
case "$norm" in
  ...
  status)
    if [ -f "$T" ]; then
      ...
      if [ "$age" -le 900 ]; then echo "MAIN_PUSH_RESULT: main is AUTHORIZED (window open ~$(( (900 - age) / 60 ))m more; covers merge+push)."
      else rm -f "$T"; echo "MAIN_PUSH_RESULT: main is NOT authorized (previous token expired)."; fi
    else echo "MAIN_PUSH_RESULT: main is NOT authorized (default). Pushes/merges to main are blocked."; fi
    ;;
  *)
    mkdir -p "$QHOME"; : > "$T"
    echo "MAIN_PUSH_RESULT: main AUTHORIZED for a 15-minute release window (covers the merge and the push). Proceed with the main release now. Revoke early with /main-push off."
    ;;
esac
