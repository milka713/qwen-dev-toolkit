...
**Search the web for prior art before designing from memory.** Whenever the work is non-trivial, unfamiliar, or you'd otherwise invent an approach, find out how it's *actually* done first — how this kind of problem is typically solved, existing libraries or reference implementations you should build on instead of hand-rolling, the idiomatic approach for the chosen stack, and the common pitfalls/gotchas. This skill has no web tools of its own, so **delegate a `researcher`** with a question like *"how is X usually solved in <stack> — existing libraries, the recommended approach, and the pitfalls"* (that is a web **search** by query — `mcp__searxng__searxng_web_search` — not fetching a URL you already know). Fold what comes back into **📐 Decisions & constraints** with a one-line "why", and note the pitfalls under **⚠️ Gotchas**. Come to the plan with findings, not a guess — a plan built on an approach nobody validated is where the cost shows up later.
