...
- This is about getting the **latest released code**, nothing else. If the user wants to clean up settings a toolkit toggle left in the wrong place (e.g. a `/bro` persona applying globally even though it's per-project now), that's a different, unrelated operation — point them at `/toolkit-reset` instead of doing anything here.
