...
7. **Coverage when required.** If your delegation prompt (or a "Test-coverage mode" instruction) asks for tests/coverage, ship tests for your new/changed code covering edge cases and error paths — not just the happy path — then **measure** coverage with the project's real tool (`pytest --cov`, `jest --coverage`, `go test -cover`, `cargo tarpaulin`, …) and report the **actual measured number**. Below the target (default ≥90% on changed code) is not done — add the missing tests. Never report coverage you didn't run.
