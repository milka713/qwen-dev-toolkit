...
1. **You own exactly one task.** Do the task you were given completely. Do not expand scope, refactor unrelated code, or start the "next" task — that belongs to a different delegation. If the task is ambiguous on a detail, take the simplest interpretation that fits the codebase and record it in `NOTES`; report `STATUS: blocked` only for a genuine either/or the main session must decide.
...
6. **Fix what you broke — and search before you guess.** If a check fails, debug and fix it within this task. When you hit an **unfamiliar error, a library API/signature you're unsure of, or a build failure with no obvious fix, search the web before guessing or reporting blocked** — paste the distinctive error text or the exact symbol into **`mcp__searxng__searxng_web_search`** (a query search; `web_fetch` only opens a URL you already have and is NOT searching). **Search FIRST, then fetch the hit** — don't `web_fetch` a docs URL guessed from memory instead of searching, and do this even for an error with **no code** (its message text is the query). Read the authoritative hit (`web_fetch` / `mcp__searxng__web_url_read`), then act on what it says. Reconstructing an API from memory or retrying the same failing command blindly is the anti-pattern. (If no `*_web_search` tool is available this session, note that and proceed from local sources — don't loop on a missing tool.) Only report `STATUS: blocked` on a failure you cannot resolve **after** looking it up — and include what the search turned up plus what you tried.
7. **Don't touch version control or secrets.** Never `git commit`, `git push`, or switch branches — version control belongs to the main session. Never hardcode a key/token/password (the `secret-guard` hook blocks it anyway): read it from env/config and add a placeholder to `.env.example` if one is needed.
8. **Tests / coverage when required.** If your delegation prompt (or a "Test-first / coverage mode" instruction) asks for tests, work **test-first** where practical (write the failing test, watch it fail, implement, watch it pass, refactor). Cover edge cases and error paths, not just the happy path, then **measure** coverage with the project's real tool (`pytest --cov`, `jest --coverage`, `go test -cover`, `cargo tarpaulin`, …) and report the **actual measured number**. Below the target stated in the delegation (the coverage mode's target %, default 80% on changed code) is not done — add the missing tests. Never report coverage you didn't run.
