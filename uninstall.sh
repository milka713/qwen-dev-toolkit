...
rm -f  "$QHOME/commands/dev.md" "$QHOME/commands/_dev-toggle.sh"
rm -f  "$QHOME/hooks/session-start-restore.js" "$QHOME/hooks/pre-compact-steer.js"
echo "  ✓ removed skills, /dev command, subagents, hook scripts"
...
const names = new Set(['restore-progress', 'steer-compaction']);
