...
const rules = [
  ...
  // A CODELESS but distinctive failure message (no ERR_/TS#### code) is still one search away —
  // this is the case the model tends to answer by `web_fetch`-ing a guessed docs URL instead of
  // actually searching. The fragments below are specific real error strings, not casual words.
  [/\b(executable does ?n'?t exist|cannot find module|module not found|no such file or directory|command not found|permission denied|unexpected token|cannot read propert(y|ies) of (undefined|null)|is not a function|connection refused|econnrefused|address already in use|eaddrinuse|segmentation fault|core dumped|maximum call stack|heap out of memory|failed to (compile|resolve|load|fetch|connect|start))\b/, 'search the web for that exact failure message before theorising — it is distinctive, so paste it verbatim into the `*_web_search` tool (MCP search is prefixed, e.g. `mcp__searxng__searxng_web_search`) FIRST, then `web_fetch`/`web_url_read` the best hit; do NOT fetch a guessed URL instead of searching'],
  [/(не найден модуль|модуль не найден|нет такого файла|команда не найдена|отказано в доступе|отказано в соединении|адрес уже используется|переполнение стека|недостаточно памяти|не является функцией|не удалось (собрать|разрешить|загрузить|подключ|запустить|скомпилир))/, 'search the web for that exact failure message before theorising — it is distinctive, so paste it verbatim into the `*_web_search` tool (MCP search is prefixed, e.g. `mcp__searxng__searxng_web_search`) FIRST, then `web_fetch`/`web_url_read` the best hit; do NOT fetch a guessed URL instead of searching'],
